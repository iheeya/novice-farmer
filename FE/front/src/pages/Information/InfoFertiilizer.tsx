...
import { getInfoFertilizer } from "../../services/Information/InfoApi";
import { GetImage } from "../../services/getImage";
import BookIcon from "../../assets/icons/Book.png"; // 기본 아이콘 이미지 경로 설정
import { useNavigate } from "react-router-dom";

// 데이터 타입 정의
interface FertilizerDataProps {
  title: string;
  comment: string;
  imageName: string;
...
export default function InfoFertilizer() {
  const [fertilizerData, setFertilizerData] = useState<FertilizerDataProps[]>([]); // 데이터 상태 관리
  const [loading, setLoading] = useState(true); // 로딩 상태 관리
  const navigate = useNavigate();

  // S3 이미지 URL을 가져오는 비동기 함수
  const fetchImageUrls = async (data: FertilizerDataProps[] = []) => {
    ...
    try {
      ...
      setFertilizerData(updatedData); // 상태 업데이트
    } catch (error) {
  ...
  const handleClickOpen = (title: string) => {
    navigate(`/info/plant/fertilizer/${encodeURIComponent(title)}`); // URL 이동
  };
            ...
            >
              <ListItemIcon>
                {item.imageUrl ? (
                  ...
                  <img src={BookIcon} width="40" height="40" alt="책 아이콘" /> // 기본 아이콘 표시
                )}
              </ListItemIcon>
              <ListItemText
                primary={item.title}
